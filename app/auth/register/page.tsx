...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error: err } = await signUp(formData.email, formData.password, formData.username);
    if (err) {
      setError(err);
      setLoading(false);
    } else {
        ...
        <div className="elegant-card p-8">
          ...
          <div className="space-y-3 mb-6">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 border border-slate-300 rounded-lg text-slate-700 bg-white hover:bg-slate-50 transition-colors disabled:opacity-50 text-sm font-medium"
            >
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="px-3 bg-white text-muted-foreground">或使用邮箱注册</span>
            </div>
          </div>
