...
async function crawlWebPages(searchResults: SearchResult[]): Promise<DetailedResult[]> {
  ...
  // 实际爬取需要 Puppeteer/Playwright，这里先返回原结果
 ...
 * 使用 ZAI API 生成智能总结
 */
async function generateAISummary(detailedResults: DetailedResult[], summaryLength: number): Promise<AISummary> {
...
`;
  ...
  try {
    const response = await fetch('https://open.bigmodel.cn/api/paas/v4/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: '你是一位专业的建筑学专家，擅长建筑案例分析和总结。请以 JSON 格式输出分析结果。'
          },
          {
            role: 'user',
            content: prompt,
    ...
    console.error('[Smart Search] ZAI API error:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 步骤 3：生成 AI 总结
    console.log('[Smart Search] Step 3: Generating AI summary...');
    const aiSummary = await generateAISummary(detailedResults, summary_length);
